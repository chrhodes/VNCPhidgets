...
namespace VNC.Phidget22.Ex
{
    public class IREx : Phidgets.IR, INotifyPropertyChanged
    {
        ...
        public IREx(IRConfiguration configuration, IEventAggregator eventAggregator)
        {
            long startTicks = 0;
        ...
        private void InitializePhidget(IRConfiguration configuration)
        {
            ...
            Detach += IREx_Detach;
            Error += IREx_Error;
            PropertyChange += IREx_PropertyChange;
        ...
        private void IREx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"IREx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, "ERRORS");
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit IREx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, "ERRORS");
        ...
        private void IREx_PropertyChange(object sender, PhidgetsEvents.PropertyChangeEventArgs e)
        {
            if (LogPropertyChangeEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"IREx_PropertyChange: sender:{sender} {e.PropertyName}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, "ERRORS");
            ...
            switch (e.PropertyName)
            {
                ...
                default:
                    Log.EVENT_HANDLER($"IREx_PropertyChange: sender:{sender} {e.PropertyName} - Update switch()", Common.LOG_CATEGORY);
                    break;
            }
        }

        private void IREx_Detach(object sender, PhidgetsEvents.DetachEventArgs e)
        {
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"IREx_Detach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, "ERRORS");
        ...
        private void IREx_Error(object sender, PhidgetsEvents.ErrorEventArgs e)
        {
            if (LogErrorEvents)
            {
                try
            ...
            try
            {
                if (LogChannelAction)
                {
                    startTicks = Log.TRACE(
                         $"RunActionLoops(>{irSequence.Name}<)" +
                         $" startActionLoopSequences:>{irSequence.StartActionLoopSequences?.Count()}<" +
                         $" actionLoops:>{irSequence.ActionLoops}<" +
                         $" serialNumber:>{DeviceSerialNumber}<" +
                         $" hubPort:>{HubPort}< >{irSequence.HubPort}<" +
                         $" channel:>{Channel}< >{irSequence.Channel}<" +
                         $" actions:>{irSequence.Actions?.Count()}<" +
                         $" actionsDuration:>{irSequence.ActionsDuration}<" +
                         $" endActionLoopSequences:>{irSequence.EndActionLoopSequences?.Count()}<" +
                         $" thread:>{System.Environment.CurrentManagedThreadId}<", Common.LOG_CATEGORY);
                }

                if (irSequence.Actions is not null)
                {
                    for (Int32 actionLoop = 0; actionLoop < irSequence.ActionLoops; actionLoop++)
                    {
                        ...
                        if (irSequence.ExecuteActionsInParallel)
                        {
                            ...
                            Parallel.ForEach(irSequence.Actions, action =>
                            {
                        ...
                        if (irSequence.ActionsDuration is not null)
                        {
                            if (LogChannelAction)
                            {
                                Log.TRACE($"Zzzz End of Actions" +
                                    $" Sleeping:>{irSequence.ActionsDuration}<", Common.LOG_CATEGORY);
                            }
        ...
        private void ExecuteAction(IRAction action)
        {
            Int64 startTicks = 0;
