    ...
    public class SoundSensorEx : Phidgets.SoundSensor, INotifyPropertyChanged
    {
        ...
        private void SoundSensorEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"SoundSensorEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit SoundSensorEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
                ...
                if (soundSensorSequence.Actions is not null)
                {
                    for (Int32 actionLoop = 0; actionLoop < soundSensorSequence.ActionLoops; actionLoop++)
                    {
                        ...
                        if (soundSensorSequence.ExecuteActionsInParallel)
                        {
                            ...
                            await Parallel.ForEachAsync(soundSensorSequence.Actions, cancellationToken, async (action, token) =>
                            {
