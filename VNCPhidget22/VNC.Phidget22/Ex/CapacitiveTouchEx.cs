...
namespace VNC.Phidget22.Ex
{
    public class CapacitiveTouchEx : Phidgets.CapacitiveTouch, INotifyPropertyChanged
    {
        ...
        private void InitializePhidget(CapacitiveTouchConfiguration configuration)
        {
            ...
            Attach += CapacitiveTouchEx_Attach;
            Detach += CapacitiveTouchEx_Detach;
            Error += CapacitiveTouchEx_Error;
            PropertyChange += CapacitiveTouchEx_PropertyChange;
        ...
        private void CapacitiveTouchEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"CapacitiveTouchEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit CapacitiveTouchEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
        ...
        private void CapacitiveTouchEx_PropertyChange(object sender, PhidgetsEvents.PropertyChangeEventArgs e)
        {
            if (LogPropertyChangeEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"CapacitiveTouchEx_PropertyChange: sender:{sender} {e.PropertyName}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
            ...
            switch (e.PropertyName)
            {
                ...
                default:
                    Log.EVENT_HANDLER($"CapacitiveTouchEx_PropertyChange: sender:{sender} {e.PropertyName} - Update switch()", Common.LOG_CATEGORY);
                    break;
            }
        }

        private void CapacitiveTouchEx_Detach(object sender, PhidgetsEvents.DetachEventArgs e)
        {
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"CapacitiveTouchEx_Detach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
        ...
        private void CapacitiveTouchEx_Error(object sender, PhidgetsEvents.ErrorEventArgs e)
        {
            if (LogErrorEvents)
            {
                try
                ...
                if (capacitiveTouchSequence.Actions is not null)
                {
                    for (Int32 actionLoop = 0; actionLoop < capacitiveTouchSequence.ActionLoops; actionLoop++)
                    {
                        ...
                        if (capacitiveTouchSequence.ExecuteActionsInParallel)
                        {
                            ...
                            await Parallel.ForEachAsync(capacitiveTouchSequence.Actions, cancellationToken, async (action, token) =>
                            {
                                ExecuteAction(action);
                                await Task.CompletedTask;
                            });

                            //Parallel.ForEach(capacitiveTouchSequence.Actions, action =>
                            //{
                            //    ExecuteAction(action);
                            //});
                        }
        ...
        private void ExecuteAction(CapacitiveTouchAction action)
        {
            Int64 startTicks = 0;
        ...
        private async void TriggerSequence(SequenceEventArgs args)
        {
            ...
            var sequence = args.CapacitiveTouchSequence;
