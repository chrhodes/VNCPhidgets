...
namespace VNC.Phidget22.Ex
{
    public class RFIDEx : Phidgets.RFID, INotifyPropertyChanged
    {
        ...
        private readonly RFIDConfiguration _RFIDConfiguration;
        ...
        public RFIDEx(int serialNumber, RFIDConfiguration RFIDConfiguration, IEventAggregator eventAggregator)
        {
            long startTicks = 0;
            ...
            _RFIDConfiguration = RFIDConfiguration;
        ...
        private void InitializePhidget()
        {
            ...
            Channel = _RFIDConfiguration.Channel;
            IsRemote = true;
                ...
                if (RFIDSequence.Actions is not null)
                {
                    for (int actionLoop = 0; actionLoop < RFIDSequence.ActionLoops; actionLoop++)
                    {
                        if (RFIDSequence.StartActionLoopSequences is not null)
                        {
                            ...
                            foreach (PerformanceSequence sequence in RFIDSequence.StartActionLoopSequences)
                            {
                        ...
                        if (RFIDSequence.EndActionLoopSequences is not null)
                        {
                            ...
                            foreach (PerformanceSequence sequence in RFIDSequence.EndActionLoopSequences)
                            {
                                await player.ExecutePerformanceSequence(sequence);
                            }
                        }
