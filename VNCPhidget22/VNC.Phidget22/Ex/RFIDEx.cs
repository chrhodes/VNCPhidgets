...
namespace VNC.Phidget22.Ex
{
    public class RFIDEx : Phidgets.RFID, INotifyPropertyChanged
    {
        ...
        private void InitializePhidget(RFIDConfiguration configuration)
        {
            ...
            Attach += RFIDEx_Attach;
            Detach += RFIDEx_Detach;
            Error += RFIDEx_Error;
            PropertyChange += RFIDEx_PropertyChange;
        ...
        private void RFIDEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"RFIDEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit RFIDEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
        ...
        private void RFIDEx_PropertyChange(object sender, PhidgetsEvents.PropertyChangeEventArgs e)
        {
            if (LogPropertyChangeEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"RFIDEx_PropertyChange: sender:{sender} {e.PropertyName}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
            ...
            switch (e.PropertyName)
            {
                ...
                default:
                    Log.EVENT_HANDLER($"RFIDEx_PropertyChange: sender:{sender} {e.PropertyName} - Update switch()", Common.LOG_CATEGORY);
                    break;
            }
        }

        private void RFIDEx_Detach(object sender, PhidgetsEvents.DetachEventArgs e)
        {
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"RFIDEx_Detach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
        ...
        private void RFIDEx_Error(object sender, PhidgetsEvents.ErrorEventArgs e)
        {
            if (LogErrorEvents)
            {
                try
        ...
        {
            Int64 startTicks = 0;
            if (LogPhidgetEvents) startTicks = Log.TRACE($"Enter attached:{base.Attached} isOpen:{IsOpen} ", Common.LOG_CATEGORY); if (LogPhidgetEvents) startTicks = Log.TRACE($"Enter attached:{base.Attached} isOpen:{IsOpen} " +
                $" s#:{DeviceSerialNumber} hubport:{HubPort} channel:{Channel}", Common.LOG_CATEGORY);
            base.Close();
                ...
                if (rfidSequence.Actions is not null)
                {
                    for (Int32 actionLoop = 0; actionLoop < rfidSequence.ActionLoops; actionLoop++)
                    {
                        ...
                        if (rfidSequence.ExecuteActionsInParallel)
                        {
                            ...
                            await Parallel.ForEachAsync(rfidSequence.Actions, cancellationToken, async (action, token) =>
                            {
