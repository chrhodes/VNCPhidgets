...
namespace VNC.Phidget22.Ex
{
    public class EncoderEx : Phidgets.Encoder, INotifyPropertyChanged
    {
        ...
        private void InitializePhidget(EncoderConfiguration configuration)
        {
            ...
            Attach += EncoderEx_Attach;
            Detach += EncoderEx_Detach;
            Error += EncoderEx_Error;
            PropertyChange += EncoderEx_PropertyChange;
        ...
        private void EncoderEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"EncoderEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, "ERRORS");
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit EncoderEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, "ERRORS");
        ...
        private void EncoderEx_PropertyChange(object sender, PhidgetsEvents.PropertyChangeEventArgs e)
        {
            if (LogPropertyChangeEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"EncoderEx_PropertyChange: sender:{sender} {e.PropertyName}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, "ERRORS");
            ...
            switch (e.PropertyName)
            {
                ...
                default:
                    Log.EVENT_HANDLER($"EncoderEx_PropertyChange: sender:{sender} {e.PropertyName} - Update switch()", Common.LOG_CATEGORY);
                    break;
            }
        }

        private void EncoderEx_Detach(object sender, PhidgetsEvents.DetachEventArgs e)
        {
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"EncoderEx_Detach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, "ERRORS");
        ...
        private void EncoderEx_Error(object sender, PhidgetsEvents.ErrorEventArgs e)
        {
            if (LogErrorEvents)
            {
                try
                ...
                if (encoderSequence.Actions is not null)
                {
                    for (Int32 actionLoop = 0; actionLoop < encoderSequence.ActionLoops; actionLoop++)
                    {
                        ...
                        if (encoderSequence.ExecuteActionsInParallel)
                        {
                            ...
                            await Parallel.ForEachAsync(encoderSequence.Actions, cancellationToken, async (action, token) =>
                            {
                        ...
                        if (encoderSequence.ActionsDuration is not null)
                        {
                            if (LogChannelAction)
                            {
                                Log.TRACE($"Zzzz End of Actions" +
                                    $" Sleeping:>{encoderSequence.ActionsDuration}<", Common.LOG_CATEGORY);
                            }
        ...
        private void ExecuteAction(EncoderAction action)
        {
            Int64 startTicks = 0;
