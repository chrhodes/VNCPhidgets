...
namespace VNC.Phidget22.Ex
{
    public class PowerGuardEx : Phidgets.PowerGuard, INotifyPropertyChanged
    {
        ...
        public PowerGuardEx(PowerGuardConfiguration configuration, IEventAggregator eventAggregator)
        {
            long startTicks = 0;
        ...
        private void InitializePhidget(PowerGuardConfiguration configuration)
        {
            ...
            Attach += PowerGuardEx_Attach;
            Detach += PowerGuardEx_Detach;
            Error += PowerGuardEx_Error;
            PropertyChange += PowerGuardEx_PropertyChange;
        ...
        private void PowerGuardEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"PowerGuardEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit PowerGuardEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
        ...
        private void PowerGuardEx_PropertyChange(object sender, PhidgetsEvents.PropertyChangeEventArgs e)
        {
            if (LogPropertyChangeEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"PowerGuardEx_PropertyChange: sender:{sender} {e.PropertyName}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
            ...
            switch (e.PropertyName)
            {
                ...
                default:
                    Log.EVENT_HANDLER($"PowerGuardEx_PropertyChange: sender:{sender} {e.PropertyName} - Update switch()", Common.LOG_CATEGORY);
                    break;
            }
        }

        private void PowerGuardEx_Detach(object sender, PhidgetsEvents.DetachEventArgs e)
        {
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"PowerGuardEx_Detach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
        ...
        private void PowerGuardEx_Error(object sender, PhidgetsEvents.ErrorEventArgs e)
        {
            if (LogErrorEvents)
            {
                try
                ...
                if (powerGuardSequence.Actions is not null)
                {
                    for (Int32 actionLoop = 0; actionLoop < powerGuardSequence.ActionLoops; actionLoop++)
                    {
                        ...
                        if (powerGuardSequence.ExecuteActionsInParallel)
                        {
                            ...
                            await Parallel.ForEachAsync(powerGuardSequence.Actions, cancellationToken, async (action, token) =>
                            {
        ...
        private async void TriggerSequence(SequenceEventArgs args)
        {
            ...
            var sequence = args.PowerGuardSequence;
