...
{
    public class GyroscopeEx : Phidgets.Gyroscope, INotifyPropertyChanged
    {
        ...
        /// <summary>
        /// Initializes a new Gyroscope and adds Event handlers
        /// </summary>
        /// <param name="serialNumber"></param>
        /// <param name="GyroscopeConfiguration"></param>
        /// <param name="eventAggregator"></param>
        public GyroscopeEx(GyroscopeConfiguration configuration, IEventAggregator eventAggregator)
        {
            long startTicks = 0;
            ...
            _eventAggregator.GetEvent<GyroscopeSequenceEvent>().Subscribe(TriggerSequence);

            if (Core.Common.VNCLogging.Constructor) Log.CONSTRUCTOR("Exit", Common.LOG_CATEGORY, startTicks);
        }

        /// <summary>
        /// Configures Gyroscope using GyroscopeConfiguration
        /// and establishes event handlers
        /// </summary>
        private void InitializePhidget(GyroscopeConfiguration configuration)
        {
            long startTicks = 0;
            ...
            Attach += GyroscopeEx_Attach;
            Detach += GyroscopeEx_Detach;
            Error += GyroscopeEx_Error;
            PropertyChange += GyroscopeEx_PropertyChange;
        ...
        private void GyroscopeEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            Phidgets.Gyroscope? Gyroscope = sender as Phidgets.Gyroscope;

            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"GyroscopeEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit GyroscopeEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
        ...
        private void GyroscopeEx_PropertyChange(object sender, PhidgetsEvents.PropertyChangeEventArgs e)
        {
            if (LogPropertyChangeEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"GyroscopeEx_PropertyChange: sender:{sender} {e.PropertyName}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
            ...
            switch (e.PropertyName)
            {
                ...
                default:
                    Log.EVENT_HANDLER($"GyroscopeEx_PropertyChange: sender:{sender} {e.PropertyName} - Update switch()", Common.LOG_CATEGORY);
                    break;
            }
        }

        private void GyroscopeEx_Detach(object sender, PhidgetsEvents.DetachEventArgs e)
        {
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"GyroscopeEx_Detach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
        ...
        private void GyroscopeEx_Error(object sender, PhidgetsEvents.ErrorEventArgs e)
        {
            if (LogErrorEvents)
            {
                try
            ...
            try
            {
                if (LogChannelAction)
                {
                    startTicks = Log.TRACE(
                         $"RunActionLoops(>{gyroscopeSequence.Name}<)" +
                         $" startActionLoopSequences:>{gyroscopeSequence.StartActionLoopSequences?.Count()}<" +
                         $" actionLoops:>{gyroscopeSequence.ActionLoops}<" +
                         $" serialNumber:>{DeviceSerialNumber}<" +
                         $" hubPort:>{HubPort}< >{gyroscopeSequence.HubPort}<" +
                         $" channel:>{Channel}< >{gyroscopeSequence.Channel}<" +
                         $" actions:>{gyroscopeSequence.Actions?.Count()}<" +
                         $" actionsDuration:>{gyroscopeSequence.ActionsDuration}<" +
                         $" endActionLoopSequences:>{gyroscopeSequence.EndActionLoopSequences?.Count()}<" +
                         $" thread:>{System.Environment.CurrentManagedThreadId}<", Common.LOG_CATEGORY);
                }

                if (gyroscopeSequence.Actions is not null)
                {
                    for (Int32 actionLoop = 0; actionLoop < gyroscopeSequence.ActionLoops; actionLoop++)
                    {
                        ...
                        if (gyroscopeSequence.ExecuteActionsInParallel)
                        {
                            ...
                            await Parallel.ForEachAsync(gyroscopeSequence.Actions, cancellationToken, async (action, token) =>
                            {
                                ExecuteAction(action);
                                await Task.CompletedTask;

                            });
                            //Parallel.ForEach(gyroscopeSequence.Actions, action =>
                            //{
                            //    ExecuteAction(action);
                            //});
                        }
                        ...
                        if (gyroscopeSequence.ActionsDuration is not null)
                        {
                            if (LogChannelAction)
                            {
                                Log.TRACE($"Zzzz End of Actions" +
                                    $" Sleeping:>{gyroscopeSequence.ActionsDuration}<", Common.LOG_CATEGORY);
                            }
        ...
        private void ExecuteAction(GyroscopeAction action)
        {
            Int64 startTicks = 0;
        ...
        private async void TriggerSequence(SequenceEventArgs args)
        {
            ...
            var sequence = args.GyroscopeSequence;
