...
namespace VNC.Phidget22.Ex
{
    public class GyroscopeEx : Phidgets.Gyroscope, INotifyPropertyChanged
    {
        ...
        private readonly GyroscopeConfiguration _GyroscopeConfiguration;
        ...
        public GyroscopeEx(int serialNumber, GyroscopeConfiguration GyroscopeConfiguration, IEventAggregator eventAggregator)
        {
            long startTicks = 0;
            ...
            _GyroscopeConfiguration = GyroscopeConfiguration;
        ...
        private void InitializePhidget()
        {
            ...
            Channel = _GyroscopeConfiguration.Channel;
            IsRemote = true;
        ...
        private void GyroscopeEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"GyroscopeEx_Attach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit GyroscopeEx_Attach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.Error(ex, Common.LOG_CATEGORY);
                ...
                if (GyroscopeSequence.Actions is not null)
                {
                    for (int actionLoop = 0; actionLoop < GyroscopeSequence.ActionLoops; actionLoop++)
                    {
                        if (GyroscopeSequence.StartActionLoopSequences is not null)
                        {
                            ...
                            foreach (DeviceClassSequence sequence in GyroscopeSequence.StartActionLoopSequences)
                            {
                        ...
                        if (GyroscopeSequence.EndActionLoopSequences is not null)
                        {
                            ...
                            foreach (DeviceClassSequence sequence in GyroscopeSequence.EndActionLoopSequences)
                            {
                                await player.ExecutePerformanceSequence(sequence);
                            }
                        }
