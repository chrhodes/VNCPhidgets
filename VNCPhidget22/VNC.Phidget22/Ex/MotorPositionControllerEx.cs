...
namespace VNC.Phidget22.Ex
{
    public class MotorPositionControllerEx : Phidgets.MotorPositionController, INotifyPropertyChanged
    {
        ...
        public MotorPositionControllerEx(MotorPositionControllerConfiguration configuration, IEventAggregator eventAggregator)
        {
            long startTicks = 0;
        ...
        /// </summary>
        private void InitializePhidget(MotorPositionControllerConfiguration configuration)
        {
            long startTicks = 0;
            ...
            Attach += MotorPositionControllerEx_Attach;
            Detach += MotorPositionControllerEx_Detach;
            Error += MotorPositionControllerEx_Error;
            PropertyChange += MotorPositionControllerEx_PropertyChange;
        ...
        private void MotorPositionControllerEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"MotorPositionControllerEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, "ERRORS");
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit MotorPositionControllerEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, "ERRORS");
        ...
        private void MotorPositionControllerEx_PropertyChange(object sender, PhidgetsEvents.PropertyChangeEventArgs e)
        {
            if (LogPropertyChangeEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"MotorPositionControllerEx_PropertyChange: sender:{sender} {e.PropertyName}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, "ERRORS");
            ...
            switch (e.PropertyName)
            {
                ...
                default:
                    Log.EVENT_HANDLER($"MotorPositionControllerEx_PropertyChange: sender:{sender} {e.PropertyName} - Update switch()", Common.LOG_CATEGORY);
                    break;
            }
        }

        private void MotorPositionControllerEx_Detach(object sender, PhidgetsEvents.DetachEventArgs e)
        {
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"MotorPositionControllerEx_Detach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, "ERRORS");
        ...
        private void MotorPositionControllerEx_Error(object sender, PhidgetsEvents.ErrorEventArgs e)
        {
            if (LogErrorEvents)
            {
                try
            ...
            try
            {
                if (LogChannelAction)
                {
                    startTicks = Log.TRACE(
                        $"RunActionLoops(>{motorPositionControllerSequence.Name}<)" +
                        ...
                        $" serialNumber:>{DeviceSerialNumber}<" +
                        $" hubPort:>{HubPort}< >{motorPositionControllerSequence.HubPort}<" +
                        $" channel:>{Channel}< >{motorPositionControllerSequence.Channel}<" +
                        $" actions:>{motorPositionControllerSequence.Actions?.Count()}<" +
                        $" actionsDuration:>{motorPositionControllerSequence.ActionsDuration}<" +
                        $" endActionLoopSequences:>{motorPositionControllerSequence.EndActionLoopSequences?.Count()}<" +
                        $" thread:>{System.Environment.CurrentManagedThreadId}<", Common.LOG_CATEGORY);
                }

                if (motorPositionControllerSequence.Actions is not null)
                {
                    for (Int32 actionLoop = 0; actionLoop < motorPositionControllerSequence.ActionLoops; actionLoop++)
                    {
                        ...
                        if (motorPositionControllerSequence.ExecuteActionsInParallel)
                        {
                            ...
                            await Parallel.ForEachAsync(motorPositionControllerSequence.Actions, cancellationToken, async (action, token) =>
                            {
                                ExecuteAction(action);
                                await Task.CompletedTask;

                            });
                            //Parallel.ForEach(motorPositionControllerSequence.Actions, action =>
                            //{
                            //    ExecuteAction(action);
                            //});
                        }
        ...
        private void ExecuteAction(MotorPositionControllerAction action)
        {
            Int64 startTicks = 0;
        ...
        private async void TriggerSequence(SequenceEventArgs args)
        {
            ...
            var sequence = args.MotorPositionControllerSequence;
