...
{
    public class BLDCMotorEx : Phidgets.BLDCMotor, INotifyPropertyChanged
    {
        #region Constructors, Initialization, and Load

        private readonly BLDCMotorConfiguration _BLDCMotorConfiguration;
        private readonly IEventAggregator _eventAggregator;

        /// <summary>
        /// Initializes a new BLDCMotorEx and adds Event handlers
        /// </summary>
        /// <param name="serialNumber"></param>
        /// <param name="BLDCMotorConfiguration"></param>
        /// <param name="eventAggregator"></param>
        public BLDCMotorEx(int serialNumber, BLDCMotorConfiguration BLDCMotorConfiguration, IEventAggregator eventAggregator)
        {
            long startTicks = 0;
            ...
            _BLDCMotorConfiguration = BLDCMotorConfiguration;
            _eventAggregator = eventAggregator;

            InitializePhidget();

            _eventAggregator.GetEvent<BLDCMotorSequenceEvent>().Subscribe(TriggerSequence);
        ...
        /// <summary>
        /// Configures BLDCMotorEx using BLDCMotorConfiguration
        ...
        private void InitializePhidget()
        {
            ...
            Channel = _BLDCMotorConfiguration.Channel;
            IsRemote = true;

            Attach += BLDCMotorExEx_Attach;
            Detach += BLDCMotorExEx_Detach;
            Error += BLDCMotorExEx_Error;
            PropertyChange += BLDCMotorExEx_PropertyChange;
        ...
        private void BLDCMotorExEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            Phidgets.BLDCMotor BLDCMotorEx = sender as Phidgets.BLDCMotor;

            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"BLDCMotorExEx_Attach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit BLDCMotorExEx_Attach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.Error(ex, Common.LOG_CATEGORY);
        ...
        private void BLDCMotorExEx_PropertyChange(object sender, PhidgetsEvents.PropertyChangeEventArgs e)
        {
            if (LogPropertyChangeEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"BLDCMotorExEx_PropertyChange: sender:{sender} {e.PropertyName}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.Error(ex, Common.LOG_CATEGORY);
            ...
            switch (e.PropertyName)
            {
                ...
                default:
                    Log.EVENT_HANDLER($"BLDCMotorExEx_PropertyChange: sender:{sender} {e.PropertyName} - Update switch()", Common.LOG_CATEGORY);
                    break;
            }
        }

        private void BLDCMotorExEx_Detach(object sender, PhidgetsEvents.DetachEventArgs e)
        {
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"BLDCMotorExEx_Detach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.Error(ex, Common.LOG_CATEGORY);
        ...
        private void BLDCMotorExEx_Error(object sender, PhidgetsEvents.ErrorEventArgs e)
        {
            if (LogErrorEvents)
            {
                try
                ...
                if (BLDCMotorSequence.Actions is not null)
                {
                    for (int actionLoop = 0; actionLoop < BLDCMotorSequence.ActionLoops; actionLoop++)
                    {
                        if (BLDCMotorSequence.StartActionLoopSequences is not null)
                        {
                            ...
                            foreach (PerformanceSequence sequence in BLDCMotorSequence.StartActionLoopSequences)
                            {
                        ...
                        if (BLDCMotorSequence.EndActionLoopSequences is not null)
                        {
                            ...
                            foreach (PerformanceSequence sequence in BLDCMotorSequence.EndActionLoopSequences)
                            {
                                await player.ExecutePerformanceSequence(sequence);
                            }
                        }
        ...
        private async Task PerformAction(BLDCMotorAction action)
        {
            ...
            if (LogSequenceAction)
            {
                startTicks = Log.Trace($"Enter BLDCMotorEx:{Channel}", Common.LOG_CATEGORY);
                actionMessage.Append($"BLDCMotorEx:{Channel}");
            }
