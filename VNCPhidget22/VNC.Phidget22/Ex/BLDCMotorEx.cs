...
namespace VNC.Phidget22.Ex
{
    public class BLDCMotorEx : Phidgets.BLDCMotor, INotifyPropertyChanged
    {
        ...
        public BLDCMotorEx(BLDCMotorConfiguration configuration, IEventAggregator eventAggregator)
        {
            long startTicks = 0;
        ...
        /// </summary>
        private void InitializePhidget(BLDCMotorConfiguration configuration)
        {
            long startTicks = 0;
            ...
            Attach += BLDCMotorEx_Attach;
            Detach += BLDCMotorEx_Detach;
            Error += BLDCMotorEx_Error;
            PropertyChange += BLDCMotorEx_PropertyChange;
        ...
        private void BLDCMotorEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"BLDCMotorEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit BLDCMotorEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
        ...
        private void BLDCMotorEx_PropertyChange(object sender, PhidgetsEvents.PropertyChangeEventArgs e)
        {
            if (LogPropertyChangeEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"BLDCMotorEx_PropertyChange: sender:{sender} {e.PropertyName}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
            ...
            switch (e.PropertyName)
            {
                ...
                default:
                    Log.EVENT_HANDLER($"BLDCMotorEx_PropertyChange: sender:{sender} {e.PropertyName} - Update switch()", Common.LOG_CATEGORY);
                    break;
            }
        }

        private void BLDCMotorEx_Detach(object sender, PhidgetsEvents.DetachEventArgs e)
        {
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"BLDCMotorEx_Detach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
        ...
        private void BLDCMotorEx_Error(object sender, PhidgetsEvents.ErrorEventArgs e)
        {
            if (LogErrorEvents)
            {
                try
        ...
        public async Task RunActionLoops(BLDCMotorSequence bldcMotorSequence)
        {
            ...
            try
            {
                ...
                {
                    for (Int32 actionLoop = 0; actionLoop < bldcMotorSequence.ActionLoops; actionLoop++)
                    {
                        ...
                        if (bldcMotorSequence.ExecuteActionsInParallel)
                        {
                            ...
                            await Parallel.ForEachAsync(bldcMotorSequence.Actions, cancellationToken, async (action, token) =>
                            {
        ...
        private async void TriggerSequence(SequenceEventArgs args)
        {
            ...
            var sequence = args.BLDCMotorSequence;

            if (sequence != null) await RunActionLoops(sequence);
