...
namespace VNC.Phidget22.Ex
{
    public class PHSensorEx : Phidgets.PHSensor, INotifyPropertyChanged
    {
        ...
        public PHSensorEx(PHSensorConfiguration configuration, IEventAggregator eventAggregator)
        {
            long startTicks = 0;
        ...
        private void InitializePhidget(PHSensorConfiguration configuration)
        {
            ...
            Attach += PHSensorEx_Attach;
            Detach += PHSensorEx_Detach;
            Error += PHSensorEx_Error;
            PropertyChange += PHSensorEx_PropertyChange;
        ...
        private void PHSensorEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"PHSensorEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit PHSensorEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
        ...
        private void PHSensorEx_PropertyChange(object sender, PhidgetsEvents.PropertyChangeEventArgs e)
        {
            if (LogPropertyChangeEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"PHSensorEx_PropertyChange: sender:{sender} {e.PropertyName}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
            ...
            switch (e.PropertyName)
            {
                ...
                default:
                    Log.EVENT_HANDLER($"PHSensorEx_PropertyChange: sender:{sender} {e.PropertyName} - Update switch()", Common.LOG_CATEGORY);
                    break;
            }
        }

        private void PHSensorEx_Detach(object sender, PhidgetsEvents.DetachEventArgs e)
        {
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"PHSensorEx_Detach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
        ...
        private void PHSensorEx_Error(object sender, PhidgetsEvents.ErrorEventArgs e)
        {
            if (LogErrorEvents)
            {
                try
            ...
            try
            {
                if (LogChannelAction)
                {
                    startTicks = Log.TRACE(
                        $"RunActionLoops(>{phSensorSequence.Name}<)" +
                        $" startActionLoopSequences:>{phSensorSequence.StartActionLoopSequences?.Count()}<" +
                        $" actionLoops:>{phSensorSequence.ActionLoops}<" +
                        $" serialNumber:>{DeviceSerialNumber}<" +
                        $" hubPort:>{HubPort}< >{phSensorSequence.HubPort}<" +
                        $" channel:>{Channel}< >{phSensorSequence.Channel}<" +
                        $" actions:>{phSensorSequence.Actions?.Count()}<" +
                        $" actionsDuration:>{phSensorSequence.ActionsDuration}<" +
                        $" endActionLoopSequences:>{phSensorSequence.EndActionLoopSequences?.Count()}<" +
                        $" thread:>{System.Environment.CurrentManagedThreadId}<", Common.LOG_CATEGORY);
                }

                if (phSensorSequence.Actions is not null)
                {
                    for (Int32 actionLoop = 0; actionLoop < phSensorSequence.ActionLoops; actionLoop++)
                    {
                        if (phSensorSequence.StartActionLoopSequences is not null)
                        {
                            ...
                            foreach (DeviceChannelSequence sequence in phSensorSequence.StartActionLoopSequences)
                            {
                        ...
                        if (phSensorSequence.ExecuteActionsInParallel)
                        {
                            ...
                            await Parallel.ForEachAsync(phSensorSequence.Actions, cancellationToken, async (action, token) =>
                            {
                                ExecuteAction(action);
                                await Task.CompletedTask;

                            });
                            //Parallel.ForEach(phSensorSequence.Actions, action =>
                            //{
                            //    ExecuteAction(action);
                            //});
                        }
                        ...
                        if (phSensorSequence.ActionsDuration is not null)
                        {
                            if (LogChannelAction)
                            {
                                Log.TRACE($"Zzzz End of Actions" +
                                    $" Sleeping:>{phSensorSequence.ActionsDuration}<", Common.LOG_CATEGORY);
                            }
                        ...
                        if (phSensorSequence.EndActionLoopSequences is not null)
                        {
                            ...
                            foreach (DeviceChannelSequence sequence in phSensorSequence.EndActionLoopSequences)
                            {
                                await player.ExecuteDeviceChannelSequence(sequence);
                            }
                        }
        ...
        private void ExecuteAction(PHSensorAction action)
        {
            Int64 startTicks = 0;
        ...
        private async void TriggerSequence(SequenceEventArgs args)
        {
            ...
            var sequence = args.PHSensorSequence;
