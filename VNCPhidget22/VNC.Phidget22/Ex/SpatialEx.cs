...
namespace VNC.Phidget22.Ex
{
    public class SpatialEx : Phidgets.Spatial, INotifyPropertyChanged
    {
        ...
        private void InitializePhidget(SpatialConfiguration configuration)
        {
            ...
            Attach += SpatialEx_Attach;
            Detach += SpatialEx_Detach;
            Error += SpatialEx_Error;
            PropertyChange += SpatialEx_PropertyChange;
        ...
        private void SpatialEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"SpatialEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, "ERRORS");
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit SpatialEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, "ERRORS");
        ...
        private void SpatialEx_PropertyChange(object sender, PhidgetsEvents.PropertyChangeEventArgs e)
        {
            if (LogPropertyChangeEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"SpatialEx_PropertyChange: sender:{sender} {e.PropertyName}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, "ERRORS");
            ...
            switch (e.PropertyName)
            {
                ...
                default:
                    Log.EVENT_HANDLER($"SpatialEx_PropertyChange: sender:{sender} {e.PropertyName} - Update switch()", Common.LOG_CATEGORY);
                    break;
            }
        }

        private void SpatialEx_Detach(object sender, PhidgetsEvents.DetachEventArgs e)
        {
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"SpatialEx_Detach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, "ERRORS");
        ...
        private void SpatialEx_Error(object sender, PhidgetsEvents.ErrorEventArgs e)
        {
            if (LogErrorEvents)
            {
                try
                ...
                if (spatialSequence.Actions is not null)
                {
                    for (Int32 actionLoop = 0; actionLoop < spatialSequence.ActionLoops; actionLoop++)
                    {
                        ...
                        if (spatialSequence.ExecuteActionsInParallel)
                        {
                            ...
                            await Parallel.ForEachAsync(spatialSequence.Actions, cancellationToken, async (action, token) =>
                            {
