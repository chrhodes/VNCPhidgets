...
namespace VNC.Phidget22.Ex
{
    public class AccelerometerEx : Phidgets.Accelerometer, INotifyPropertyChanged
    {
        ...
        public AccelerometerEx(AccelerometerConfiguration configuration, IEventAggregator eventAggregator)
        {
            long startTicks = 0;
        ...
        private void AccelerometerEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"AccelerometerEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit AccelerometerEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
        ...
        public async Task RunActionLoops(AccelerometerSequence accelerometerSequence)
        {
            ...
            try
            {
                ...
                {
                    for (Int32 actionLoop = 0; actionLoop < accelerometerSequence.ActionLoops; actionLoop++)
                    {
                        ...
                        if (accelerometerSequence.ActionsDuration is not null)
                        {
                            if (LogChannelAction)
                            {
                                Log.TRACE($"Zzzz End of Actions" +
                                    $" Sleeping:>{accelerometerSequence.ActionsDuration}<", Common.LOG_CATEGORY);
                            }
                        ...
                        if (accelerometerSequence.EndActionLoopSequences is not null)
                        {
                            ...
                            foreach (DeviceChannelSequence sequence in accelerometerSequence.EndActionLoopSequences)
                            {
                                await player.ExecuteDeviceChannelSequence(sequence);
                            }
                        }
