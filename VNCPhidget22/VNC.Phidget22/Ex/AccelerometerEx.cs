...
{
    public class AccelerometerEx : Phidgets.Accelerometer, INotifyPropertyChanged
    {
        #region Constructors, Initialization, and Load

        private readonly AccelerometerConfiguration _accelerometerConfiguration;
        private readonly IEventAggregator _eventAggregator;

        /// <summary>
        /// Initializes a new Accelerometer and adds Event handlers
        /// </summary>
        /// <param name="serialNumber"></param>
        /// <param name="accelerometerConfiguration"></param>
        /// <param name="eventAggregator"></param>
        public AccelerometerEx(int serialNumber, AccelerometerConfiguration accelerometerConfiguration, IEventAggregator eventAggregator)
        {
            long startTicks = 0;
            ...
            _accelerometerConfiguration = accelerometerConfiguration;
            _eventAggregator = eventAggregator;

            InitializePhidget();

            _eventAggregator.GetEvent<AccelerometerSequenceEvent>().Subscribe(TriggerSequence);
        ...
        private void InitializePhidget()
        {
            ...
            Channel = _accelerometerConfiguration.Channel;
            IsRemote = true;
        ...
        private void AccelerometerEx_PropertyChange(object sender, PhidgetsEvents.PropertyChangeEventArgs e)
        {
            if (LogPropertyChangeEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"AccelerometerEx_PropertyChange: sender:{sender} {e.PropertyName}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.Error(ex, Common.LOG_CATEGORY);
            ...
            switch (e.PropertyName)
            {
                ...
                default:
                    Log.EVENT_HANDLER($"AccelerometerEx_PropertyChange: sender:{sender} {e.PropertyName} - Update switch()", Common.LOG_CATEGORY);
                    break;
            }
        }

        private void AccelerometerEx_Detach(object sender, PhidgetsEvents.DetachEventArgs e)
        {
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"AccelerometerEx_Detach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.Error(ex, Common.LOG_CATEGORY);
        ...
        private void AccelerometerEx_Error(object sender, PhidgetsEvents.ErrorEventArgs e)
        {
            if (LogErrorEvents)
            {
                try
            ...
            try
            {
                ...
                if (LogPerformanceSequence)
                {
                    startTicks = Log.Trace(
                        ...
                        $" accelerometerSequence:>{accelerometerSequence.Name}<" +
                ...
                {
                    for (int actionLoop = 0; actionLoop < accelerometerSequence.ActionLoops; actionLoop++)
                    {
                        if (accelerometerSequence.StartActionLoopSequences is not null)
                        {
                            // TODO(crhodes)
                            ...
                            foreach (PhidgetDeviceClassSequence sequence in accelerometerSequence.StartActionLoopSequences)
                            {
                        ...
                        if (accelerometerSequence.ActionsDuration is not null)
                        {
                            if (LogSequenceAction)
                            {
                                Log.Trace($"Zzzzz Action:>{accelerometerSequence.ActionsDuration}<", Common.LOG_CATEGORY);
                            }

                            Thread.Sleep((Int32)accelerometerSequence.ActionsDuration);
                        }

                        if (accelerometerSequence.EndActionLoopSequences is not null)
                        {
                            PhidgetDeviceSequencePlayer player = new PhidgetDeviceSequencePlayer(_eventAggregator);
                            player.LogPerformanceSequence = LogPerformanceSequence;
                            player.LogSequenceAction = LogSequenceAction;

                            foreach (PhidgetDeviceClassSequence sequence in accelerometerSequence.EndActionLoopSequences)
                            {
                                await player.ExecutePhidgetDeviceSequence(sequence);
                            }
                        }
