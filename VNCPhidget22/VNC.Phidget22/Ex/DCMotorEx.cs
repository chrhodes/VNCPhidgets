...
namespace VNC.Phidget22.Ex
{
    public class DCMotorEx : Phidgets.DCMotor, INotifyPropertyChanged
    {
        ...
        public DCMotorEx(DCMotorConfiguration configuration, IEventAggregator eventAggregator)
        {
            long startTicks = 0;
        ...
        /// </summary>
        private void InitializePhidget(DCMotorConfiguration configuration)
        {
            long startTicks = 0;
            ...
            Attach += DCMotorEx_Attach;
            Detach += DCMotorEx_Detach;
            Error += DCMotorEx_Error;
            PropertyChange += DCMotorEx_PropertyChange;
        ...
        private void DCMotorEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"DCMotorEx_Attach: sender:{sender} isAttached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit DCMotorEx_Attach: sender:{sender} isAttached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.Error(ex, Common.LOG_CATEGORY);
        ...
        private void DCMotorEx_PropertyChange(object sender, PhidgetsEvents.PropertyChangeEventArgs e)
        {
            if (LogPropertyChangeEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"DCMotorEx_PropertyChange: sender:{sender} {e.PropertyName}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.Error(ex, Common.LOG_CATEGORY);
            ...
            switch (e.PropertyName)
            {
                ...
                default:
                    Log.EVENT_HANDLER($"DCMotorEx_PropertyChange: sender:{sender} {e.PropertyName} - Update switch()", Common.LOG_CATEGORY);
                    break;
            }
        }

        private void DCMotorEx_Detach(object sender, PhidgetsEvents.DetachEventArgs e)
        {
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"DCMotorEx_Detach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.Error(ex, Common.LOG_CATEGORY);
        ...
        private void DCMotorEx_Error(object sender, PhidgetsEvents.ErrorEventArgs e)
        {
            if (LogErrorEvents)
            {
                try
            ...
            try
            {
                if (LogChannelAction)
                {
                    startTicks = Log.Trace(
                        $"RunActionLoops(>{dcMotorSequence.Name}<)" +
                        $" startActionLoopSequences:>{dcMotorSequence.StartActionLoopSequences?.Count()}<" +
                        $" actionLoops:>{dcMotorSequence.ActionLoops}<" +
                        $" serialNumber:>{DeviceSerialNumber}<" +
                        $" hubPort:>{HubPort}< >{dcMotorSequence.HubPort}<" +
                        $" channel:>{Channel}< >{dcMotorSequence.Channel}<" +
                        $" actions:>{dcMotorSequence.Actions?.Count()}<" +
                        $" actionsDuration:>{dcMotorSequence.ActionsDuration}<" +
                        $" endActionLoopSequences:>{dcMotorSequence.EndActionLoopSequences?.Count()}<" +
                        $" thread:>{System.Environment.CurrentManagedThreadId}<", Common.LOG_CATEGORY);
                }

                if (dcMotorSequence.Actions is not null)
                {
                    for (Int32 actionLoop = 0; actionLoop < dcMotorSequence.ActionLoops; actionLoop++)
                    {
        ...
        private async void TriggerSequence(SequenceEventArgs args)
        {
            ...
            var sequence = args.DCMotorSequence;
