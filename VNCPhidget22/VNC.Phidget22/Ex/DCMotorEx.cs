...
{
    public class DCMotorEx : Phidgets.DCMotor, INotifyPropertyChanged
    {
        #region Constructors, Initialization, and Load

        private readonly DCMotorConfiguration _DCMotorConfiguration;
        private readonly IEventAggregator _eventAggregator;

        /// <summary>
        /// Initializes a new DCMotorEx and adds Event handlers
        /// </summary>
        /// <param name="serialNumber"></param>
        /// <param name="DCMotorConfiguration"></param>
        /// <param name="eventAggregator"></param>
        public DCMotorEx(int serialNumber, DCMotorConfiguration DCMotorConfiguration, IEventAggregator eventAggregator)
        {
            long startTicks = 0;
            ...
            _DCMotorConfiguration = DCMotorConfiguration;
            _eventAggregator = eventAggregator;

            InitializePhidget();

            _eventAggregator.GetEvent<DCMotorSequenceEvent>().Subscribe(TriggerSequence);
        ...
        /// <summary>
        /// Configures DCMotorEx using DCMotorConfiguration
        ...
        private void InitializePhidget()
        {
            ...
            Channel = _DCMotorConfiguration.Channel;
            IsRemote = true;

            Attach += DCMotorExEx_Attach;
            Detach += DCMotorExEx_Detach;
            Error += DCMotorExEx_Error;
            PropertyChange += DCMotorExEx_PropertyChange;
        ...
        private void DCMotorExEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            Phidgets.DCMotor DCMotor = sender as Phidgets.DCMotor;

            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"DCMotorExEx_Attach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit DCMotorExEx_Attach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.Error(ex, Common.LOG_CATEGORY);
        ...
        private void DCMotorExEx_PropertyChange(object sender, PhidgetsEvents.PropertyChangeEventArgs e)
        {
            if (LogPropertyChangeEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"DCMotorExEx_PropertyChange: sender:{sender} {e.PropertyName}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.Error(ex, Common.LOG_CATEGORY);
            ...
            switch (e.PropertyName)
            {
                ...
                default:
                    Log.EVENT_HANDLER($"DCMotorExEx_PropertyChange: sender:{sender} {e.PropertyName} - Update switch()", Common.LOG_CATEGORY);
                    break;
            }
        }

        private void DCMotorExEx_Detach(object sender, PhidgetsEvents.DetachEventArgs e)
        {
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"DCMotorExEx_Detach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.Error(ex, Common.LOG_CATEGORY);
        ...
        private void DCMotorExEx_Error(object sender, PhidgetsEvents.ErrorEventArgs e)
        {
            if (LogErrorEvents)
            {
                try
        ...
        public async Task RunActionLoops(DCMotorSequence DCMotorSequence)
        {
            try
            {
                Int64 startTicks = 0;
                ...
                if (LogPerformanceSequence)
                {
                    startTicks = Log.Trace(
                        ...
                        $" DCMotorSequence:>{DCMotorSequence.Name}<" +
                        $" startActionLoopSequences:>{DCMotorSequence.StartActionLoopSequences?.Count()}<" +
                        $" actionLoops:>{DCMotorSequence.ActionLoops}<" +
                        $" actions:>{DCMotorSequence.Actions.Count()}<" +
                        $" actionsDuration:>{DCMotorSequence?.ActionsDuration}<" +
                        $" endActionLoopSequences:>{DCMotorSequence.EndActionLoopSequences?.Count()}<", Common.LOG_CATEGORY);
                }

                if (DCMotorSequence.Actions is not null)
                {
                    for (int actionLoop = 0; actionLoop < DCMotorSequence.ActionLoops; actionLoop++)
                    {
                        if (DCMotorSequence.StartActionLoopSequences is not null)
                        {
                            // TODO(crhodes)
                            ...
                            foreach (PhidgetDeviceClassSequence sequence in DCMotorSequence.StartActionLoopSequences)
                            {
                                await player.ExecutePhidgetDeviceSequence(sequence);
                            }
                        }

                        if (DCMotorSequence.ExecuteActionsInParallel)
                        {
                            if (LogSequenceAction) Log.Trace($"Parallel Actions Loop:>{actionLoop + 1}<", Common.LOG_CATEGORY);

                            Parallel.ForEach(DCMotorSequence.Actions, async action =>
                            {
                        ...
                        else
                        {
                            ...
                            foreach (DCMotorAction action in DCMotorSequence.Actions)
                            {
                                await PerformAction(action);
                            }
                        }

                        if (DCMotorSequence.ActionsDuration is not null)
                        {
                            if (LogSequenceAction)
                            {
                                Log.Trace($"Zzzzz Action:>{DCMotorSequence.ActionsDuration}<", Common.LOG_CATEGORY);
                            }

                            Thread.Sleep((Int32)DCMotorSequence.ActionsDuration);
                        }

                        if (DCMotorSequence.EndActionLoopSequences is not null)
                        {
                            PhidgetDeviceSequencePlayer player = new PhidgetDeviceSequencePlayer(_eventAggregator);
                            player.LogPerformanceSequence = LogPerformanceSequence;
                            player.LogSequenceAction = LogSequenceAction;

                            foreach (PhidgetDeviceClassSequence sequence in DCMotorSequence.EndActionLoopSequences)
                            {
                                await player.ExecutePhidgetDeviceSequence(sequence);
                            }
                        }
        ...
        // FIX(crhodes)
        // 
        private async Task PerformAction(DCMotorAction action)
        {
            Int64 startTicks = 0;
            ...
            if (LogSequenceAction)
            {
                startTicks = Log.Trace($"Enter DCMotorEx:{Channel}", Common.LOG_CATEGORY);
                actionMessage.Append($"DCMotorEx:{Channel}");
            }
