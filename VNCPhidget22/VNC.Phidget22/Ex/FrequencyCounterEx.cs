...
namespace VNC.Phidget22.Ex
{
    public class FrequencyCounterEx : Phidgets.FrequencyCounter, INotifyPropertyChanged
    {
        ...
        private void InitializePhidget(FrequencyCounterConfiguration configuration)
        {
            ...
            Attach += FrequencyCounterEx_Attach;
            Detach += FrequencyCounterEx_Detach;
            Error += FrequencyCounterEx_Error;
            PropertyChange += FrequencyCounterEx_PropertyChange;
        ...
        private void FrequencyCounterEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"FrequencyCounterEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit FrequencyCounterEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
        ...
        private void FrequencyCounterEx_PropertyChange(object sender, PhidgetsEvents.PropertyChangeEventArgs e)
        {
            if (LogPropertyChangeEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"FrequencyCounterEx_PropertyChange: sender:{sender} {e.PropertyName}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
            ...
            switch (e.PropertyName)
            {
                ...
                default:
                    Log.EVENT_HANDLER($"FrequencyCounterEx_PropertyChange: sender:{sender} {e.PropertyName} - Update switch()", Common.LOG_CATEGORY);
                    break;
            }
        }

        private void FrequencyCounterEx_Detach(object sender, PhidgetsEvents.DetachEventArgs e)
        {
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"FrequencyCounterEx_Detach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
        ...
        private void FrequencyCounterEx_Error(object sender, PhidgetsEvents.ErrorEventArgs e)
        {
            if (LogErrorEvents)
            {
                try
                ...
                if (frequencyCounterSequence.Actions is not null)
                {
                    for (Int32 actionLoop = 0; actionLoop < frequencyCounterSequence.ActionLoops; actionLoop++)
                    {
                        ...
                        if (frequencyCounterSequence.ExecuteActionsInParallel)
                        {
                            ...
                            await Parallel.ForEachAsync(frequencyCounterSequence.Actions, cancellationToken, async (action, token) =>
                            {
        ...
        private async void TriggerSequence(SequenceEventArgs args)
        {
            ...
            var sequence = args.FrequencyCounterSequence;
