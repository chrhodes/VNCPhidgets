...
{
    public class FrequencyCounterEx : Phidgets.FrequencyCounter, INotifyPropertyChanged
    {
        #region Constructors, Initialization, and Load

        private readonly FrequencyCounterConfiguration _FrequencyCounterConfiguration;
        private readonly IEventAggregator _eventAggregator;

        /// <summary>
        /// Initializes a new FrequencyCounterEx and adds Event handlers
        /// </summary>
        /// <param name="serialNumber"></param>
        /// <param name="FrequencyCounterConfiguration"></param>
        /// <param name="eventAggregator"></param>
        public FrequencyCounterEx(int serialNumber, FrequencyCounterConfiguration FrequencyCounterConfiguration, IEventAggregator eventAggregator)
        {
            long startTicks = 0;
            ...
            _FrequencyCounterConfiguration = FrequencyCounterConfiguration;
            _eventAggregator = eventAggregator;

            InitializePhidget();

            _eventAggregator.GetEvent<FrequencyCounterSequenceEvent>().Subscribe(TriggerSequence);
        ...
        /// <summary>
        /// Configures FrequencyCounterEx using FrequencyCounterConfiguration
        ...
        private void InitializePhidget()
        {
            ...
            Channel = _FrequencyCounterConfiguration.Channel;
            IsRemote = true;

            Attach += FrequencyCounterExEx_Attach;
            Detach += FrequencyCounterExEx_Detach;
            Error += FrequencyCounterExEx_Error;
            PropertyChange += FrequencyCounterExEx_PropertyChange;
        ...
        private void FrequencyCounterExEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            Phidgets.FrequencyCounter FrequencyCounter = sender as Phidgets.FrequencyCounter;

            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"FrequencyCounterExEx_Attach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit FrequencyCounterExEx_Attach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.Error(ex, Common.LOG_CATEGORY);
        ...
        private void FrequencyCounterExEx_PropertyChange(object sender, PhidgetsEvents.PropertyChangeEventArgs e)
        {
            if (LogPropertyChangeEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"FrequencyCounterExEx_PropertyChange: sender:{sender} {e.PropertyName}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.Error(ex, Common.LOG_CATEGORY);
            ...
            switch (e.PropertyName)
            {
                ...
                default:
                    Log.EVENT_HANDLER($"FrequencyCounterExEx_PropertyChange: sender:{sender} {e.PropertyName} - Update switch()", Common.LOG_CATEGORY);
                    break;
            }
        }

        private void FrequencyCounterExEx_Detach(object sender, PhidgetsEvents.DetachEventArgs e)
        {
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"FrequencyCounterExEx_Detach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.Error(ex, Common.LOG_CATEGORY);
        ...
        private void FrequencyCounterExEx_Error(object sender, PhidgetsEvents.ErrorEventArgs e)
        {
            if (LogErrorEvents)
            {
                try
        ...
        public async Task RunActionLoops(FrequencyCounterSequence FrequencyCounterSequence)
        {
            try
            {
                Int64 startTicks = 0;
                ...
                if (LogPerformanceSequence)
                {
                    startTicks = Log.Trace(
                        ...
                        $" FrequencyCounterSequence:>{FrequencyCounterSequence.Name}<" +
                        ...
                        $" endActionLoopSequences:>{FrequencyCounterSequence.EndActionLoopSequences?.Count()}<", Common.LOG_CATEGORY);
                }

                if (FrequencyCounterSequence.Actions is not null)
                {
                    for (int actionLoop = 0; actionLoop < FrequencyCounterSequence.ActionLoops; actionLoop++)
                    {
                        if (FrequencyCounterSequence.StartActionLoopSequences is not null)
                        {
                            // TODO(crhodes)
                            ...
                            foreach (PhidgetDeviceClassSequence sequence in FrequencyCounterSequence.StartActionLoopSequences)
                            {
                                await player.ExecutePhidgetDeviceSequence(sequence);
                            }
                        }

                        if (FrequencyCounterSequence.ExecuteActionsInParallel)
                        {
                            if (LogSequenceAction) Log.Trace($"Parallel Actions Loop:>{actionLoop + 1}<", Common.LOG_CATEGORY);

                            Parallel.ForEach(FrequencyCounterSequence.Actions, async action =>
                            {
                        ...
                        if (FrequencyCounterSequence.ActionsDuration is not null)
                        {
                        ...
                        if (FrequencyCounterSequence.EndActionLoopSequences is not null)
                        {
                            PhidgetDeviceSequencePlayer player = new PhidgetDeviceSequencePlayer(_eventAggregator);
                            player.LogPerformanceSequence = LogPerformanceSequence;
                            player.LogSequenceAction = LogSequenceAction;

                            foreach (PhidgetDeviceClassSequence sequence in FrequencyCounterSequence.EndActionLoopSequences)
                            {
                                await player.ExecutePhidgetDeviceSequence(sequence);
                            }
                        }
        ...
        private async Task PerformAction(FrequencyCounterAction action)
        {
            ...
            if (LogSequenceAction)
            {
                startTicks = Log.Trace($"Enter FrequencyCounterEx:{Channel}", Common.LOG_CATEGORY);
                actionMessage.Append($"FrequencyCounterEx:{Channel}");
            }
            ...
            try
            {
                ...
                #region FrequencyCounterEx Actions
