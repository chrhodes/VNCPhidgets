...
{
    public class GPSEx : Phidgets.GPS, INotifyPropertyChanged
    {
        ...
        /// <summary>
        /// Initializes a new GPSEx and adds Event handlers
        /// </summary>
        /// <param name="serialNumber"></param>
        /// <param name="GPSConfiguration"></param>
        /// <param name="eventAggregator"></param>
        public GPSEx(GPSConfiguration configuration, IEventAggregator eventAggregator)
        {
            long startTicks = 0;
            ...
            _eventAggregator.GetEvent<GPSSequenceEvent>().Subscribe(TriggerSequence);
        ...
        /// </summary>
        private void InitializePhidget(GPSConfiguration configuration)
        {
            long startTicks = 0;
        ...
        private void GPSEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"GPSEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit GPSEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
        ...
        public async Task RunActionLoops(GPSSequence gpsSequence)
        {
            ...
            try
            {
                ...
                {
                    for (Int32 actionLoop = 0; actionLoop < gpsSequence.ActionLoops; actionLoop++)
                    {
                        if (gpsSequence.StartActionLoopSequences is not null)
                        {
                            DeviceChannelSequencePlayer player = GetNewDeviceChannelSequencePlayer();

                            foreach (DeviceChannelSequence sequence in gpsSequence.StartActionLoopSequences)
                            {
                        ...
                        if (gpsSequence.ExecuteActionsInParallel)
                        {
                            ...
                            await Parallel.ForEachAsync(gpsSequence.Actions, cancellationToken, async (action, token) =>
                            {
                        ...
                        if (gpsSequence.ActionsDuration is not null)
                        {
                            if (LogChannelAction)
                            {
                                Log.TRACE($"Zzzz End of Actions" +
                                    $" Sleeping:>{gpsSequence.ActionsDuration}<", Common.LOG_CATEGORY);
                            }

                            Thread.Sleep((Int32)gpsSequence.ActionsDuration);
                        }

                        if (gpsSequence.EndActionLoopSequences is not null)
                        {
                            DeviceChannelSequencePlayer player = GetNewDeviceChannelSequencePlayer();

                            foreach (DeviceChannelSequence sequence in gpsSequence.EndActionLoopSequences)
                            {
                                await player.ExecuteDeviceChannelSequence(sequence);
                            }
                        }
