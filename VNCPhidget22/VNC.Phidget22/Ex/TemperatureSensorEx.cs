...
{
    public class TemperatureSensorEx : Phidgets.TemperatureSensor, INotifyPropertyChanged
    {
        #region Constructors, Initialization, and Load

        private readonly TemperatureSensorConfiguration _TemperatureSensorConfiguration;
        private readonly IEventAggregator _eventAggregator;

        /// <summary>
        /// Initializes a new TemperatureSensorEx and adds Event handlers
        /// </summary>
        /// <param name="serialNumber"></param>
        /// <param name="TemperatureSensorConfiguration"></param>
        /// <param name="eventAggregator"></param>
        public TemperatureSensorEx(int serialNumber, TemperatureSensorConfiguration TemperatureSensorConfiguration, IEventAggregator eventAggregator)
        {
            long startTicks = 0;
            ...
            _TemperatureSensorConfiguration = TemperatureSensorConfiguration;
            _eventAggregator = eventAggregator;

            InitializePhidget();

            _eventAggregator.GetEvent<TemperatureSensorSequenceEvent>().Subscribe(TriggerSequence);
        ...
        /// <summary>
        /// Configures TemperatureSensorEx using TemperatureSensorConfiguration
        ...
        private void InitializePhidget()
        {
            ...
            Channel = _TemperatureSensorConfiguration.Channel;
            IsRemote = true;
        ...
        public async Task RunActionLoops(TemperatureSensorSequence TemperatureSensorSequence)
        {
            try
            {
                Int64 startTicks = 0;
                ...
                if (LogPerformanceSequence)
                {
                    startTicks = Log.Trace(
                        ...
                        $" TemperatureSensorSequence:>{TemperatureSensorSequence.Name}<" +
                        ...
                        $" endActionLoopSequences:>{TemperatureSensorSequence.EndActionLoopSequences?.Count()}<", Common.LOG_CATEGORY);
                }

                if (TemperatureSensorSequence.Actions is not null)
                {
                    for (int actionLoop = 0; actionLoop < TemperatureSensorSequence.ActionLoops; actionLoop++)
                    {
                        if (TemperatureSensorSequence.StartActionLoopSequences is not null)
                        {
                            // TODO(crhodes)
                            ...
                            foreach (PhidgetDeviceClassSequence sequence in TemperatureSensorSequence.StartActionLoopSequences)
                            {
                                await player.ExecutePhidgetDeviceSequence(sequence);
                            }
                        }

                        if (TemperatureSensorSequence.ExecuteActionsInParallel)
                        {
                        ...
                        if (TemperatureSensorSequence.ActionsDuration is not null)
                        {
                        ...
                        if (TemperatureSensorSequence.EndActionLoopSequences is not null)
                        {
                            PhidgetDeviceSequencePlayer player = new PhidgetDeviceSequencePlayer(_eventAggregator);
                            player.LogPerformanceSequence = LogPerformanceSequence;
                            player.LogSequenceAction = LogSequenceAction;

                            foreach (PhidgetDeviceClassSequence sequence in TemperatureSensorSequence.EndActionLoopSequences)
                            {
                                await player.ExecutePhidgetDeviceSequence(sequence);
                            }
                        }
        ...
        private async Task PerformAction(TemperatureSensorAction action)
        {
            ...
            if (LogSequenceAction)
            {
                startTicks = Log.Trace($"Enter TemperatureSensorEx:{Channel}", Common.LOG_CATEGORY);
                actionMessage.Append($"TemperatureSensorEx:{Channel}");
            }
            ...
            try
            {
                ...
                #region TemperatureSensorEx Actions
