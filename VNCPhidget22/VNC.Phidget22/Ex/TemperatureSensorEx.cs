...
namespace VNC.Phidget22.Ex
{
    public class TemperatureSensorEx : Phidgets.TemperatureSensor, INotifyPropertyChanged
    {
        ...
        public TemperatureSensorEx(TemperatureSensorConfiguration configuration, IEventAggregator eventAggregator)
        {
            long startTicks = 0;
        ...
        private void TemperatureSensorEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"TemperatureSensorEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, "ERRORS");
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit TemperatureSensorEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, "ERRORS");
        ...
        public async Task RunActionLoops(TemperatureSensorSequence temperatureSensorSequence)
        {
            ...
            try
            {
                ...
                {
                    for (Int32 actionLoop = 0; actionLoop < temperatureSensorSequence.ActionLoops; actionLoop++)
                    {
                        ...
                        if (temperatureSensorSequence.ExecuteActionsInParallel)
                        {
                            ...
                            await Parallel.ForEachAsync(temperatureSensorSequence.Actions, cancellationToken, async (action, token) =>
                            {
