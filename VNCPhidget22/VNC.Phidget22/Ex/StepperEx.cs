    ...
    public class StepperEx : Phidgets.Stepper, INotifyPropertyChanged
    {
                ...
                if (stepperSequence.Actions is not null)
                {
                    for (int actionLoop = 0; actionLoop < stepperSequence.ActionLoops; actionLoop++)
                    {
                        if (stepperSequence.StartActionLoopSequences is not null)
                        {
                            ...
                            foreach (DeviceClassSequence sequence in stepperSequence.StartActionLoopSequences)
                            {
                        ...
                        if (stepperSequence.EndActionLoopSequences is not null)
                        {
                            ...
                            foreach (DeviceClassSequence sequence in stepperSequence.EndActionLoopSequences)
                            {
                                await player.ExecutePerformanceSequence(sequence);
                            }
                        }
        ...
        private async Task PerformAction(StepperAction action)
        {
            ...
            try
            {
                ...
                {
                    if (LogSequenceAction) actionMessage.Append($" rescaleFactor:>{action.StepAngle}<");
                ...
                if (action.RelativeTargetDegrees is not null)
                {
                    ...
                    if (LogSequenceAction) actionMessage.Append($" relativeTargetDegress:>{action.RelativeTargetDegrees}< ({targetPosition})");
                ...
                {
                    if (LogSequenceAction) actionMessage.Append($" addPositionOffset:>{action.AddPositionOffset}<");
