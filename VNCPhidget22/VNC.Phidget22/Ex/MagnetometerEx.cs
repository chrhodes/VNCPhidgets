...
namespace VNC.Phidget22.Ex
{
    public class MagnetometerEx : Phidgets.Magnetometer, INotifyPropertyChanged
    {
        ...
        public MagnetometerEx(MagnetometerConfiguration configuration, IEventAggregator eventAggregator)
        {
            long startTicks = 0;
        ...
        /// </summary>
        private void InitializePhidget(MagnetometerConfiguration configuration)
        {
            long startTicks = 0;
            ...
            Attach += MagnetometerEx_Attach;
            Detach += MagnetometerEx_Detach;
            Error += MagnetometerEx_Error;
            PropertyChange += MagnetometerEx_PropertyChange;
        ...
        private void MagnetometerEx_Attach(object sender, PhidgetsEvents.AttachEventArgs e)
        {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"MagnetometerEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
            ...
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"Exit MagnetometerEx_Attach: sender:{sender} attached:{Attached} isOpen:{IsOpen}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
        ...
        private void MagnetometerEx_PropertyChange(object sender, PhidgetsEvents.PropertyChangeEventArgs e)
        {
            if (LogPropertyChangeEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"MagnetometerEx_PropertyChange: sender:{sender} {e.PropertyName}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
            ...
            switch (e.PropertyName)
            {
                ...
                default:
                    Log.EVENT_HANDLER($"MagnetometerEx_PropertyChange: sender:{sender} {e.PropertyName} - Update switch()", Common.LOG_CATEGORY);
                    break;
            }
        }

        private void MagnetometerEx_Detach(object sender, PhidgetsEvents.DetachEventArgs e)
        {
            if (LogPhidgetEvents)
            {
                try
                {
                    Log.EVENT_HANDLER($"MagnetometerEx_Detach: sender:{sender}", Common.LOG_CATEGORY);
                }
                catch (Exception ex)
                {
                    Log.ERROR(ex, Common.LOG_CATEGORY);
        ...
        private void MagnetometerEx_Error(object sender, PhidgetsEvents.ErrorEventArgs e)
        {
            if (LogErrorEvents)
            {
                try
            ...
            try
            {
                if (LogChannelAction)
                {
                    startTicks = Log.TRACE(
                        $"RunActionLoops(>{magnetometerSequence.Name}<)" +
                        $" startActionLoopSequences:>{magnetometerSequence.StartActionLoopSequences?.Count()}<" +
                        $" actionLoops:>{magnetometerSequence.ActionLoops}<" +
                        $" serialNumber:>{DeviceSerialNumber}<" +
                        $" hubPort:>{HubPort}< >{magnetometerSequence.HubPort}<" +
                        $" channel:>{Channel}< >{magnetometerSequence.Channel}<" +
                        $" actions:>{magnetometerSequence.Actions?.Count()}<" +
                        $" actionsDuration:>{magnetometerSequence.ActionsDuration}<" +
                        $" endActionLoopSequences:>{magnetometerSequence.EndActionLoopSequences?.Count()}<" +
                        $" thread:>{System.Environment.CurrentManagedThreadId}<", Common.LOG_CATEGORY);
                }

                if (magnetometerSequence.Actions is not null)
                {
                    for (Int32 actionLoop = 0; actionLoop < magnetometerSequence.ActionLoops; actionLoop++)
                    {
                        if (magnetometerSequence.StartActionLoopSequences is not null)
                        {
                            ...
                            foreach (DeviceChannelSequence sequence in magnetometerSequence.StartActionLoopSequences)
                            {
                        ...
                        if (magnetometerSequence.ExecuteActionsInParallel)
                        {
                            ...
                            await Parallel.ForEachAsync(magnetometerSequence.Actions, cancellationToken, async (action, token) =>
                            {
                                ExecuteAction(action);
                                await Task.CompletedTask;

                            });
                            //Parallel.ForEach(magnetometerSequence.Actions, action =>
                            //{
                            //    ExecuteAction(action);
                            //});
                        }
                        ...
                        if (magnetometerSequence.ActionsDuration is not null)
                        {
                            if (LogChannelAction)
                            {
                                Log.TRACE($"Zzzz End of Actions" +
                                    $" Sleeping:>{magnetometerSequence.ActionsDuration}<", Common.LOG_CATEGORY);
                            }
                        ...
                        if (magnetometerSequence.EndActionLoopSequences is not null)
                        {
                            ...
                            foreach (DeviceChannelSequence sequence in magnetometerSequence.EndActionLoopSequences)
                            {
                                await player.ExecuteDeviceChannelSequence(sequence);
                            }
                        }
        ...
        private void ExecuteAction(MagnetometerAction action)
        {
            Int64 startTicks = 0;
        ...
        private async void TriggerSequence(SequenceEventArgs args)
        {
            ...
            var sequence = args.MagnetometerSequence;
