...
_setDefaults(PhidgetChannelHandle phid) {
	...
	case PHIDCHUID_DCC1001_MOTORPOSITIONCONTROLLER_100:
		...
		if (ret != EPHIDGET_OK) {
			logerr("%"PRIphid": failed to set [engaged] default: %d", phid, ret);
			break;
		}
		...
		if (ret != EPHIDGET_OK) {
			logerr("%"PRIphid": failed to set [targetPosition] default: %d", phid, ret);
			break;
		}
		ret = bridgeSendToDevice(phid, BP_SETDEADBAND, NULL, NULL, "%u", ch->deadBand);
		if (ret != EPHIDGET_OK) {
			logerr("%"PRIphid": failed to set [deadBand] default: %d", phid, ret);
			break;
		}
		ret = bridgeSendToDevice(phid, BP_SETKP, NULL, NULL, "%g", ch->kp);
		if (ret != EPHIDGET_OK) {
			logerr("%"PRIphid": failed to set [kp] default: %d", phid, ret);
			break;
		}
		ret = bridgeSendToDevice(phid, BP_SETKD, NULL, NULL, "%g", ch->kd);
		if (ret != EPHIDGET_OK) {
			logerr("%"PRIphid": failed to set [kd] default: %d", phid, ret);
			break;
		}
		ret = bridgeSendToDevice(phid, BP_SETKI, NULL, NULL, "%g", ch->ki);
		if (ret != EPHIDGET_OK) {
			logerr("%"PRIphid": failed to set [ki] default: %d", phid, ret);
			break;
		}
		break;
	case PHIDCHUID_DCC1002_MOTORPOSITIONCONTROLLER_100:
...
PhidgetMotorPositionController_setCurrentLimit(PhidgetMotorPositionControllerHandle ch,
  double currentLimit) {
	...
	TESTPTRL(ch);
	TESTCHANNELCLASSL(ch, PHIDCHCLASS_MOTORPOSITIONCONTROLLER);
	TESTATTACHEDL(ch);
