...
#ifdef COMPILE_PHIDGETS_LABVIEW
void CCONV Phidget_OnCalibrationData_LaviewHandler(PhidgetHandle ch,void *ctx, const uint8_t *data,
  size_t dataLen) {
	LVUserEventRef ev = *(LVUserEventRef *)ctx;
	MgErr ret = 0;
	lvPhidgetCalibrationDataArgs *lvdata;
	lvdata = (lvPhidgetCalibrationDataArgs *)DSNewPtr(sizeof(lvPhidgetCalibrationDataArgs));
	lvdata->ch = (intptr_t)ch;
	lvdata->data = (lvArrHandle *)DSNewHandle(sizeof(int32_t) + sizeof(uint8_t) * dataLen);
...
API_PRETURN_HDR Phidget_setOnCalibrationDataLabviewHandler(PhidgetHandle ch,
  LVUserEventRef *lvEventRef) {
	TESTPTR(ch);
	if(lvEventRef && *lvEventRef)
		Phidget_setOnCalibrationDataHandler(ch, Phidget_OnCalibrationData_LaviewHandler, lvEventRef);
	else
		Phidget_setOnCalibrationDataHandler(ch, NULL, NULL);
...
#ifdef COMPILE_PHIDGETS_LABVIEW
void CCONV PhidgetManager_OnAttach_LaviewHandler(PhidgetManagerHandle ch,void *ctx,
  PhidgetHandle channel) {
	...
	lvdata->ch = (intptr_t)ch;
	lvdata->channel = channel;
	ret = PostLVUserEvent(ev, lvdata);
	DSDisposePtr(lvdata);
}

API_PRETURN_HDR PhidgetManager_setOnAttachLabviewHandler(PhidgetManagerHandle ch,
  LVUserEventRef *lvEventRef) {
	TESTPTR(ch);
	if(lvEventRef && *lvEventRef)
		PhidgetManager_setOnAttachHandler(ch, PhidgetManager_OnAttach_LaviewHandler, lvEventRef);
	else
		PhidgetManager_setOnAttachHandler(ch, NULL, NULL);
...
#ifdef COMPILE_PHIDGETS_LABVIEW
void CCONV PhidgetManager_OnDetach_LaviewHandler(PhidgetManagerHandle ch,void *ctx,
  PhidgetHandle channel) {
	LVUserEventRef ev = *(LVUserEventRef *)ctx;
	MgErr ret = 0;
...
API_PRETURN_HDR PhidgetManager_setOnDetachLabviewHandler(PhidgetManagerHandle ch,
  LVUserEventRef *lvEventRef) {
	TESTPTR(ch);
	if(lvEventRef && *lvEventRef)
		PhidgetManager_setOnDetachHandler(ch, PhidgetManager_OnDetach_LaviewHandler, lvEventRef);
	else
		PhidgetManager_setOnDetachHandler(ch, NULL, NULL);
...
void CCONV PhidgetAccelerometer_OnAccelerationChange_LaviewHandler(PhidgetAccelerometerHandle ch,
 void *ctx, const double acceleration[3], double timestamp) {
	...
	lvdata->ch = (intptr_t)ch;
	lvdata->acceleration = (lvArrHandle *)DSNewHandle(sizeof(int32_t) + sizeof(double) * 3);
...
void CCONV PhidgetGyroscope_OnAngularRateUpdate_LaviewHandler(PhidgetGyroscopeHandle ch,void *ctx,
  const double angularRate[3], double timestamp) {
	...
	lvdata->ch = (intptr_t)ch;
	lvdata->angularRate = (lvArrHandle *)DSNewHandle(sizeof(int32_t) + sizeof(double) * 3);
...
void CCONV PhidgetMagnetometer_OnMagneticFieldChange_LaviewHandler(PhidgetMagnetometerHandle ch,void *ctx,
  const double magneticField[3], double timestamp) {
	...
	lvdata->ch = (intptr_t)ch;
	lvdata->magneticField = (lvArrHandle *)DSNewHandle(sizeof(int32_t) + sizeof(double) * 3);
...
void CCONV PhidgetIR_OnRawData_LaviewHandler(PhidgetIRHandle ch,void *ctx, const uint32_t *data,
  size_t dataLen) {
	...
	lvdata->ch = (intptr_t)ch;
	lvdata->data = (lvArrHandle *)DSNewHandle(sizeof(int32_t) + sizeof(uint32_t) * dataLen);
...
void CCONV PhidgetDistanceSensor_OnSonarReflectionsUpdate_LaviewHandler(PhidgetDistanceSensorHandle ch,
 void *ctx, const uint32_t distances[8], const uint32_t amplitudes[8], uint32_t count) {
	...
	lvdata->ch = (intptr_t)ch;
	lvdata->distances = (lvArrHandle *)DSNewHandle(sizeof(int32_t) + sizeof(uint32_t) * 8);
	memcpy(&((*(lvdata->distances))->data), distances, sizeof(uint32_t) * 8);
	(*(lvdata->distances))->length = (int32_t)8;
	lvdata->amplitudes = (lvArrHandle *)DSNewHandle(sizeof(int32_t) + sizeof(uint32_t) * 8);
...
void CCONV PhidgetSoundSensor_OnSPLChange_LaviewHandler(PhidgetSoundSensorHandle ch,void *ctx, double dB,
  double dBA, double dBC, const double octaves[10]) {
	...
	lvdata->dBA = dBA;
	lvdata->dBC = dBC;
	lvdata->octaves = (lvArrHandle *)DSNewHandle(sizeof(int32_t) + sizeof(double) * 10);
...
#ifdef COMPILE_PHIDGETS_LABVIEW
void CCONV PhidgetGeneric_OnPacket_LaviewHandler(PhidgetGenericHandle ch,void *ctx, const uint8_t *packet,
  size_t packetLen) {
	LVUserEventRef ev = *(LVUserEventRef *)ctx;
	MgErr ret = 0;
	lvPhidgetGenericPacketArgs *lvdata;
	lvdata = (lvPhidgetGenericPacketArgs *)DSNewPtr(sizeof(lvPhidgetGenericPacketArgs));
	lvdata->ch = (intptr_t)ch;
	lvdata->packet = (lvArrHandle *)DSNewHandle(sizeof(int32_t) + sizeof(uint8_t) * packetLen);
	memcpy(&((*(lvdata->packet))->data), packet, sizeof(uint8_t) * packetLen);
	(*(lvdata->packet))->length = (int32_t)packetLen;
	ret = PostLVUserEvent(ev, lvdata);
	DSDisposePtr(lvdata);
}

API_PRETURN_HDR PhidgetGeneric_setOnPacketLabviewHandler(PhidgetGenericHandle ch,
  LVUserEventRef *lvEventRef) {
	TESTPTR(ch);
	if(lvEventRef && *lvEventRef)
		PhidgetGeneric_setOnPacketHandler(ch, PhidgetGeneric_OnPacket_LaviewHandler, lvEventRef);
	else
		PhidgetGeneric_setOnPacketHandler(ch, NULL, NULL);
