...
PhidgetInit() {
	...
	mos_tlock_init(netAttachDetachQueueLock, P22LOCK_DEVICELISTLOCK, P22LOCK_FLAGS);
}
...
Phidget_getAttached(PhidgetHandle deviceOrChannel, int *status) {
	...
	*status = ISATTACHED(deviceOrChannel);
	return (EPHIDGET_OK);
}
...
API_PRETURN
Phidget_calibrate(PhidgetHandle phid, unsigned char offset, unsigned char data[6]) {
	PhidgetChannelHandle channel;
	...
	return (PhidgetDevice_sendpacket(channel->parent, buffer, 8));
}

API_PRETURN
Phidget_calibrateGainOffset2(PhidgetHandle phid, int Index, uint16_t offset, uint32_t gain1, uint32_t gain2) {
	PhidgetChannelHandle channel;
	...
	res = PhidgetDevice_sendpacket(channel->parent, buffer, 3);
	if (res != EPHIDGET_OK)
		return (res);
...
API_PRETURN
Phidget_calibrateGainOffset(PhidgetHandle phid, int Index, uint16_t offset, uint32_t gain) {
	PhidgetChannelHandle channel;
