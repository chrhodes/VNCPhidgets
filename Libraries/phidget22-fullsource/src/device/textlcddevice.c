...
// === Internal Functions === //
static PhidgetReturnCode _sendpacket(PhidgetTextLCDDeviceHandle phid, int Index);
static void escapeLcdString(char *string, size_t len, char *buffer, int *buf_ptr);
static PhidgetReturnCode _writeString(PhidgetTextLCDDeviceHandle phid, int screen, int Row, int Column, const char *chars);
static PhidgetReturnCode writeScreen(PhidgetTextLCDDeviceHandle phid, int Index);
static PhidgetReturnCode _setCustomCharacter(PhidgetTextLCDDeviceHandle phid, int screen, const char *charStr, const uint8_t *bitmap);
static PhidgetReturnCode writeBuffer(PhidgetTextLCDDeviceHandle phid, int packet_type, uint8_t *buffer, int buflen, int screen);
...
PhidgetTextLCDDevice_initAfterOpen(PhidgetDeviceHandle device) {
	...
	for (i = 0; i < phid->devChannelCnts.numScreens; i++) {
		...
		ret = writeBuffer(phid, 0, buffer, 2, i);
		if (ret != EPHIDGET_OK)
			return ret;
...
PhidgetTextLCDDevice_bridgeInput(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHCLASS_DIGITALOUTPUT:
		...
		case BP_SETSTATE:
			...
			return (_sendpacket(phid, (ch->index << 8) | TEXTLCD_DIGITALOUTPUT_PACKET));
		case BP_OPENRESET:
		case BP_CLOSERESET:
	...
	case PHIDCHCLASS_LCD:
		...
		case BP_SETBACKLIGHT:
			...
			return (_sendpacket(phid, (ch->index << 8) | TEXTLCD_BACKLIGHT_PACKET));

		case BP_SETCURSORON:
			phid->cursorOn[ch->index] = getBridgePacketInt32(bp, 0);
			return (_sendpacket(phid, (ch->index << 8) | TEXTLCD_CURSOR_PACKET));
		...
		case BP_SETSCREENSIZE:
			...
			TESTRANGE(screenSize, SCREEN_SIZE_NONE, SCREEN_SIZE_4x40);
			...
			// flush
			return (writeScreen(phid, ch->index));

		case BP_SETCONTRAST:
			phid->_contrastSet[ch->index] = getBridgePacketDouble(bp, 0);
			if (!phid->fullStateEcho)
				phid->contrast[ch->index] = phid->_contrastSet[ch->index];
			return (_sendpacket(phid, (ch->index << 8) | TEXTLCD_CONTRAST_PACKET));

		case BP_WRITETEXT:
			if (phid->screenSize[ch->index] == (PhidgetLCD_ScreenSize)PUNK_ENUM)
				return (EPHIDGET_NOTCONFIGURED);
			return (_writeString(phid, ch->index, getBridgePacketInt32(bp, 2), getBridgePacketInt32(bp, 1), getBridgePacketString(bp, 3)));
		...
		case BP_SETCHARACTERBITMAP:
			...
			return (_setCustomCharacter(phid, ch->index, getBridgePacketString(bp, 1), getBridgePacketUInt8Array(bp, 2)));
		...
		case BP_INITIALIZE:
			...
			return (_sendpacket(phid, (ch->index << 8) | TEXTLCD_INIT_PACKET));
		...
		case BP_CLOSERESET:
			...
			res = _sendpacket(phid, (ch->index << 8) | TEXTLCD_CURSOR_PACKET);
			if (res != EPHIDGET_OK)
				return (res);
			...
			res = _sendpacket(phid, (ch->index << 8) | TEXTLCD_BACKLIGHT_PACKET);
			if (res != EPHIDGET_OK)
				return (res);
			...
			phid->contrast[ch->index] = phid->_contrastSet[ch->index];
			res = _sendpacket(phid, (ch->index << 8) | TEXTLCD_CONTRAST_PACKET);
			if (res != EPHIDGET_OK)
				return (res);
...
static PhidgetReturnCode _sendpacket(PhidgetTextLCDDeviceHandle phid, int Index) {
	...
	case PHIDUID_1219:
		...
		case TEXTLCD_BACKLIGHT_PACKET: //backlight
			...
			return (writeBuffer(phid, TEXTLCD_BACKLIGHT_PACKET, buffer, 2, screen));

		case TEXTLCD_CONTRAST_PACKET: //contrast
			buffer[0] = (uint8_t)(phid->_contrastSet[screen] * 255);

			return (writeBuffer(phid, TEXTLCD_CONTRAST_PACKET, buffer, 1, screen));
		...
		case TEXTLCD_INIT_PACKET: //re-init a screen
			...
			return (writeBuffer(phid, TEXTLCD_INIT_PACKET, buffer, 0, screen));
		...
		case TEXTLCD_CURSOR_PACKET: //LCD commands - Cursor
			...
			return (writeBuffer(phid, 0, buffer, 1, phid->cursorScreen[screen]));
		...
		case TEXTLCD_DIGITALOUTPUT_PACKET:
			...
			return (PhidgetDevice_sendpacket((PhidgetDeviceHandle)phid, buffer, getMaxOutPacketSize((PhidgetDeviceHandle)phid)));
		default:
...
writeBuffer(PhidgetTextLCDDeviceHandle phid, int packetType, uint8_t *buffer, int buflen, int screen) {
	...
	if (packetType != 0) {
		...
		ret = PhidgetDevice_sendpacket((PhidgetDeviceHandle)phid, outbuf, 8);
		if (ret != EPHIDGET_OK)
			return (ret);
	} else {
...
static PhidgetReturnCode
writeScreen(PhidgetTextLCDDeviceHandle phid, int Index) {
	...
	if (changedRows || phid->cursorOn[Index] || phid->cursorBlink[Index]) {
		...
		if (phid->cursorOn[Index] == PTRUE || phid->cursorBlink[Index] == PTRUE) {
			...
			buffer[1] = HD44780_DISPLAY_CNTRL | HD44780_DISPLAY_ON;
			ret = writeBuffer(phid, 0, buffer, 2, phid->cursorLastScreen[Index]);
			if (ret != EPHIDGET_OK)
				return (ret);
		}
		...
		for (i = 0; i < phid->height[Index]; i++) {
			if (changedRows & (0x01 << i)) {
				...
				ret = writeBuffer(phid, 0, buffer, buf_ptr, screenEnable);
				if (ret != EPHIDGET_OK)
					return (ret);
		...
		ret = writeBuffer(phid, 0, buffer, buf_ptr, phid->cursorScreen[Index]);
		if (ret != EPHIDGET_OK)
			return (ret);
...
static PhidgetReturnCode
_writeString(PhidgetTextLCDDeviceHandle phid, int screen, int Row, int Column,
  const char *chars) {
	PhidgetReturnCode ret = EPHIDGET_OK;
	int screenEnable;
	int pos;
	...
	if (Row >= phid->height[screen] || Row < 0) return (EPHIDGET_INVALID);
	if (Column >= phid->width[screen] || Column < 0) return (EPHIDGET_INVALID);
...
static PhidgetReturnCode
_setCustomCharacter(PhidgetTextLCDDeviceHandle phid, int screen, const char *character, const uint8_t *bitmap) {
	uint8_t buffer[8] = { 0 };
	PhidgetReturnCode ret = EPHIDGET_OK;
	int Val1 = 0, Val2 = 0;
	int i, j, k;
	...
	again:
		...
		ret = writeBuffer(phid, 0, buffer, 7, screenEnable);
		if (ret != EPHIDGET_OK)
			goto done;
