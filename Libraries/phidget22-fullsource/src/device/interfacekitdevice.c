...
// === Internal Functions === //
static PhidgetReturnCode _sendpacket(PhidgetInterfaceKitDeviceHandle phid, int Index);
...
PhidgetReturnCode
adjustIfkitDataIntervalsAsNeeded(PhidgetInterfaceKitDeviceHandle phid) {
	PhidgetReturnCode result = EPHIDGET_OK;
	...
	if (voltageRatioCnt > 0 && voltageInputCnt > 0)
		dataRateMax = 32;
	else
	...
	for (i = 0; i < phid->devChannelCnts.numSensors; i++) {
		...
		if ((channel = getChannel(phid, i)) != NULL) {
			...
			if (rateChanged)
				bridgeSendToChannel((PhidgetChannelHandle)channel, BP_DATAINTERVALCHANGE, "%u", dataRateMax);
	...
	if (!phid->ratiometricSwitching) {
		for (i = 0; i < phid->devChannelCnts.numSensors; i++) {
			...
			if (sensorAccumulateCount > 0) {
				phid->voltage[i] = round_double((rawSensorAccumulate / (double)sensorAccumulateCount) * 5.0 / 4095.0, 4);
				...
				if (sensorDataCount[i] > 0) {
					...
					case EVENTMODE_DATARATE:
					{
						...
						if (phid->dataInterval[i] <= phid->interruptRate) {
							for (j = 0; j < sensorDataCount[i]; j++) {
								double voltage = round_double(sensorRawValue[i][j] * 5.0 / 4095.0, 4);
...
static PhidgetReturnCode
_setDataRate(PhidgetInterfaceKitDeviceHandle phid, int Index, uint32_t newVal) {
...
PhidgetInterfaceKitDevice_bridgeInput(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHCLASS_DIGITALOUTPUT:
		...
		case BP_SETSTATE:
			...
			return (_sendpacket(phid, ch->index));
		case BP_OPENRESET:
		case BP_CLOSERESET:
	...
	case PHIDCHCLASS_VOLTAGERATIOINPUT:
		...
		case BP_SETDATAINTERVAL:
			return _setDataRate(phid, ch->index, getBridgePacketUInt32(bp, 0));
		...
		case BP_OPENRESET:
		case BP_CLOSERESET:
			return (adjustIfkitDataIntervalsAsNeeded(phid));
		case BP_ENABLE:
			return (EPHIDGET_OK);
		default:
			MOS_PANIC("Unexpected packet type");
