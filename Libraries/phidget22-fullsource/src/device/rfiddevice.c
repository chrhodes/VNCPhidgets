...
static PhidgetReturnCode rfid_write(PhidgetRFIDDeviceHandle phid, const char *tagString, PhidgetRFID_Protocol protocol, int lock);
static PhidgetReturnCode rfid_writeRaw(PhidgetRFIDDeviceHandle phid, uint8_t *data, int bitlength, int pregap, int space, int postgap, int zero, int one, int prepulse, int eof, int listenDuringEOF);
static PhidgetReturnCode _sendpacket(PhidgetRFIDDeviceHandle phid);
...
PhidgetRFIDDevice_bridgeInput(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHCLASS_RFID:
		...
		case BP_WRITE:
			TESTRANGE(getBridgePacketInt32(bp, 1), PROTOCOL_EM4100, PROTOCOL_PHIDGETS);
			return (rfid_write(phid, getBridgePacketString(bp, 0), (PhidgetRFID_Protocol)getBridgePacketInt32(bp, 1), getBridgePacketInt32(bp, 2)));
...
startTagTimerThread(PhidgetRFIDDeviceHandle phid) {
	...
	phid->tagThreadRun = 1;
	if (mos_task_create(&phid->tagTimerThread, tagTimerThreadFunction, phid) != 0) {
		phid->tagThreadRun = 0;
		mos_mutex_unlock(&phid->tagLock);
		return (EPHIDGET_UNEXPECTED);
	}
...
tagTimerThreadFunction(void *ctx) {
	...
	loginfo("tagTimerThread running");
...
static PhidgetReturnCode
waitForTag(PhidgetRFIDDeviceHandle phid, const char *tagString, int timeout) {
	//Wait for this tag to show up
	while (timeout > 0) {
		mos_mutex_lock(&phid->tagLock);
		if (!strncmp(phid->pendingTag.tagString, tagString, RFIDDevice_MAX_TAG_STRING_LEN)) {
			mos_mutex_unlock(&phid->tagLock);
			return (EPHIDGET_OK);
		}
		mos_mutex_unlock(&phid->tagLock);
...
static PhidgetReturnCode
sendRAWData(PhidgetRFIDDeviceHandle phid, uint8_t *data, int bitlength) {
	uint8_t buffer[8] = { 0 };
	int i, j, bits;
	PhidgetReturnCode result;
	...
	for (i = 0, j = 1; i < length; i++, j++) {
		...
		if (j == 7 || i == (length - 1)) {
			...
			if ((result = PhidgetDevice_sendpacket((PhidgetDeviceHandle)phid, buffer, j + 1)) != EPHIDGET_OK)
				return result;
			j = 0;
			bits = 0;
		}
...
resetValuesFromSpace(PhidgetRFIDDeviceHandle phid) {
	...
	if (phid->lastManEventLong == PFALSE) {
		...
		if ((channel = getChannel(phid, 0))) {
			bridgeSendToChannel(channel, BP_MANCHESTER, "%*R%d", 1, manEventData, 1);
			PhidgetRelease(&channel);
		}
	}
#endif
 ...
 */
static PhidgetReturnCode
encodeEM4100(const char *tagString, uint32_t *blockData, int *blockDataLen) {
 ...
 */
static PhidgetReturnCode
encodeISO11785_FDX_B(const char *tagString, uint32_t *blockData, int *blockDataLen) {
 ...
 */
static PhidgetReturnCode
encodePHIDGETS_TAG(const char *tagString, uint32_t *blockData, int *blockDataLen) {
	char tagData[24];
	int len, i, j;
	uint16_t crc;
	assert(tagString);
...
start:
	...
	if (!add_manchester_data(phid, myReadPtr)) {
		...
		if (manEventDataWritePtr) {
			phid->lastManEventLong = PFALSE;
			if ((channel = getChannel(phid, 0))) {
				bridgeSendToChannel(channel, BP_MANCHESTER, "%*R%d", manEventDataWritePtr, manEventData, manEventDataWritePtr);
				PhidgetRelease(&channel);
			}
		}
	}
...
static PhidgetReturnCode
T5577_WriteBlock(PhidgetRFIDDeviceHandle phid, int page, int block, uint32_t data, int lockpage) {
	...
	res = rfid_writeRaw(phid, byteData, 38,
...
static PhidgetReturnCode
T5577_Reset(PhidgetRFIDDeviceHandle phid) {
	PhidgetReturnCode res = EPHIDGET_OK;
	uint8_t byteData[1];

	byteData[0] = 0x00; //OpCode

	res = rfid_writeRaw(phid, byteData, 2,
		RFIDDevice_T5577_StartGap,
		RFIDDevice_T5577_WriteGap,
		RFIDDevice_T5577_EndGap,
		RFIDDevice_T5577_Zero,
 ...
 */
static PhidgetReturnCode
T5577_WriteTag(PhidgetRFIDDeviceHandle phid, PhidgetRFID_Protocol protocol, const char *tagString, int lock) {
...
static PhidgetReturnCode
rfid_writeRaw(PhidgetRFIDDeviceHandle phid, uint8_t *data, int bitlength,
	...
	case PHIDUID_1024:
		...
		//Send timing
		res = _sendpacket(phid);
		if (res != 0)
			return (res);

		res = sendRAWData(phid, data, bitlength);
		if (res != 0)
			return (res);
...
static PhidgetReturnCode
rfid_write(PhidgetRFIDDeviceHandle phid, const char *tagString, PhidgetRFID_Protocol protocol, int lock) {
	PhidgetReturnCode ret = EPHIDGET_OK;
